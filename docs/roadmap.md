...
3. M2 进行中：EdgeChat 模块化与逐条已读回执；Docker 等价部署在 M3。
...
| #21 「邮箱凭据页」A2 占位 | M6 |
| #180 管理台顶部导航末项「设置」375px 被裁切 | 待排（m1-acceptance 手机复走查发现） |
| #181 移动端触屏目标偏小（体验打磨） | 待排（m1-acceptance 手机复走查发现） |
| #182 本地验收阻断：contracts 假 provisioner 每请求新建 | 待排（m1-acceptance 手机复走查发现） |
| #184 `portalUrl` 无设置页输入位 | 待排（m1-acceptance 手机复走查发现） |
