...
| 4 | 身份 | 通用 OIDC；支持任意兼容身份源或支持 OIDC 的邮局 |
...
| 23 | 邮箱可选 | OIDC 提供方必需、邮件系统可选；完整实例（批准即 JMAP 开户）与弱化实例（无邮件，首登 JIT + email 匹配消费邀请）双形态；实例特定值（域名/发件地址）一律部署配置 |
| 24 | 邀请激活 | 邀请链接一次性/限期；批准 → 开户（随机初密）→ 激活链接发个人邮箱（一次性令牌+邮箱所有权双证）；不经管理员转交 |
