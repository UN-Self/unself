...
- [x] 平台邮件能力=运行时邮件配置（/admin/settings mail 段）+ SMTP 发信与 Stalwart 开户适配器；支撑欢迎信、通知模板（account_ready/invite_result/invite_pending）与入职激活；Git 事件投递在 M3。（2026-09-13 对齐）
...
| 23 | 邮箱可选 | 身份轴与邮件轴正交；邮件轴关闭时批准只翻状态（无开户无发信），内置模式批准即激活；实例特定值（域名/发件地址/API key）= 运行时配置（/admin/settings mail 段，即改即生效免重部署）（2026-09-13 对齐实现） |
| 24 | 邀请激活 | 邀请链接一次性/限期（默认 7 天）；批准 → 开户（随机初密）+ 后台尽力发信（发信失败不阻塞审批）；激活必经通道=新人凭邀请链接回邀请页三态自助（待审批/设置邮箱密码/去登录），claim 原子重签（单一有效链接、明文不落库）；不经管理员转交；内置无邮件模式批准即激活——注册时设密码（2026-09-13 #134 修订） |
...
| 31 | 发信定位 | CF Workers 出站 TLS 平台不可用（2026-09-13 定案，证据档案 issue #127）：服务端发信=后台尽力增强、失败不阻塞审批；激活交付=邀请页三态自助（决策 #24） |
...
2. M1 进行中：管理界面、邀请→审批→邀请页三态自助激活全链、运行时邮件配置；验收=真实邀请一人从填表到登录全程不碰 Stalwart 后台（完整版），内置无邮件实例邀请审批闭环可用（最小版）。
3. M2 起按里程碑推进：EdgeChat 模块化与逐条已读回执；Docker 等价部署在 M3。
