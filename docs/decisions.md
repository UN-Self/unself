...
| #68 | 2026-09-17 | 补 #2：一键入口重定案 | #2 记载的「一键入口（Deploy Button + Workers Builds）为路线图」**在当前架构下不可行**：Cloudflare 官方限制 monorepo 多 Worker 不能一并部署、仓库必须 public；且 CF 侧编排与 #165（setup token 只在部署者本地签发）冲突。该路线图项需重定案，本阶段不实现（原条目文字不动） | issue #241 · docs/audit 实测记录 |
