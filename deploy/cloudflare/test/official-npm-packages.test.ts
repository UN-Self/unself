...
import { modulePackageFiles, packModuleDir } from '../src/module-pack';
import { resolveSources, lockRecordFrom } from '../src/module-sources';
import { emptyLock, manifestHashOf, type LockFile } from '../src/lock';
import { previewModuleSource } from '../src/module-preview';
import { parseSource, resolveLocalNpmPackage, satisfiesVersionRange, sriFromBuffer } from '../src/sources';
import { buildManifestSnapshot } from '../src/registry';
import { writeTarball } from '../src/tar-write';
import { platformUninstallPlan } from '@unself/contracts';

/** 仓库根（真实文件布局：modules/hello、modules/chat、模块 workspace 符号链接）。 */
...
/** 官方模块安装串（与安装器 dependencies 预装版本一致，见 packages/installer/package.json）。 */
...
describe('#284 B1/B4：npm 本地命中 与 同内容 tarball 安装结果一致', () => {
  it('解析产物 / lock 投影：manifest 与 manifestHash 逐项相同（只有来源串与 SRI 不同）', async () => {
    ...
    const packed = await packModuleDir({ dir: join(ROOT, 'modules', 'hello'), outDir: out, npmName: '@unself/hello' });
