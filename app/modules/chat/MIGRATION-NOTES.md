...
- `shared/`（2）：group-channel.ts user-profile.ts（从上游仓库根复制；worker 内 `../../shared/` 相对引用在 app/modules/chat/ 下解析为 `app/modules/chat/shared/`，与上游布局一致）
...
- `test/chat-core-auth.test.ts`：正例（验签→JIT→API）+ 负例族（无 token/乱串/错签/错 aud/过期 → 401；缺 JWKS → 503）+ JIT 幂等/停用；真 ES256 keypair 手法抄 `app/modules/hello/test/hello.test.ts`
...
## 验收 grep 约定（自检 1）
`grep -rni "telegram|capacitor" app/modules/chat/` 的预期命中仅限三类说明性文字，无业务代码命中：
