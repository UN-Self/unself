...
import { packModuleDir } from '../../src/engine/module-pack';
...
describe('#284 B1/B4：npm 本地命中 与 同内容 tarball 的安装结果逐项一致', () => {
  it('注册表快照 / 路由 / 建表 / 权限投影一致；同 config 重跑零新建零重传', { timeout: 180_000 }, async () => {
    ...
    const packed = await packModuleDir({ dir: join(REPO_ROOT, 'app', 'modules', 'hello'), outDir: packOut, npmName: '@unself/hello' });
