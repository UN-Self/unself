...
describe('packModuleDir（app/modules/hello 真实模块）', () => {
  let outDir: string;
  let tarballPath: string;

  beforeAll(async () => {
    outDir = join(work, 'out-hello');
    ({ tarballPath } = await packModuleDir({ dir: join(REPO_ROOT, 'app', 'modules', 'hello'), outDir }));
  });
  ...
  it('解包出的 manifest.json 与 manifest.yaml 单轨解析 deep-equal', async () => {
    ...
    const expected = ModuleManifestSchema.parse(
      manifestFromYamlText(await readFile(join(REPO_ROOT, 'app', 'modules', 'hello', 'manifest.yaml'), 'utf8')) as Record<string, unknown>,
    );
  ...
  it('打包确定性：同一模块目录打两次 → tgz 逐字节相同且 integrity 相同', async () => {
    ...
    const again = await packModuleDir({ dir: join(REPO_ROOT, 'app', 'modules', 'hello'), outDir: out2 });
  ...
  it('worker.js 确定性（files 层）：两次 modulePackageFiles 逐字节相同', async () => {
    const a = await modulePackageFiles({ dir: join(REPO_ROOT, 'app', 'modules', 'hello') });
    const b = await modulePackageFiles({ dir: join(REPO_ROOT, 'app', 'modules', 'hello') });
...
describe('build-artifacts 产物一致性（跑过 build:artifacts 才验，否则跳过）', () => {
  const artifactsHello = join(REPO_ROOT, 'app', 'installer', 'dist', 'artifacts', 'modules', 'hello');
  ...
  it('artifacts 的 manifest.json / worker.js 与 pack 输出逐字节一致', async () => {
    ...
    const packed = await modulePackageFiles({ dir: join(REPO_ROOT, 'app', 'modules', 'hello') });
