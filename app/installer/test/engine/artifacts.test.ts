...
import { resolveArtifactRoots } from '../../src/engine/artifacts';
  ...
  it('选中模块的来源解析不了 → 人话失败且不创建任何 CF 资源（不是静默跳过）', { timeout: 120_000 }, async () => {
    ...
    await expect(
      runNineSteps({
        ...
        client: new RestClient({ token: 't', fetchImpl: fake.fetchImpl }),
        yes: true,
      configOverride: { domain: '', modules: [{ id: 'ghost', source: 'file:./app/modules/ghost' }], storage: { provider: 'r2', bucket: 'unself-storage' } },
        http: SMOKE_OK,
      }),
