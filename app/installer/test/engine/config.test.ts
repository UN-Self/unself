...
import { manifestId, moduleIds, normalizeModuleEntries, parseUnselfConfigText, stripJsonc } from '../../src/engine/config';
...
describe('modules 对象形态 {id, source}（#245/#77：只收对象）', () => {
  ...
  it('normalizeModuleEntries：对象条目归一化为 {id, source}（source 必填）', () => {
    const cfg = parseUnselfConfigText('{"modules":[{"id":"hello","source":"npm:@unself/hello@0.1.0"},{"id":"todo","source":"file:./app/modules/todo"}]}');
