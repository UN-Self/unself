...
describe('readChatPackageConfig（包配置子集解析）', () => {
  ...
  it('真实 chat 包：迁移链在标准位置 migrations/chat/（#248 存储面去豁免的锚点）', () => {
    expect(existsSync(join(REPO, 'app/modules/chat/migrations/chat/0001_baseline.sql'))).toBe(true);
    expect(existsSync(join(REPO, 'app/modules/chat/worker/schema-baseline.sql'))).toBe(false);
  });
});
...
describe('moduleWorkerEntry（模块入口解析，#219）', () => {
  ...
  it('hello：包 main = src/index.ts；chat：包 main = worker/src/index.js', async () => {
    ...
    await expect(moduleWorkerEntry(join(REPO, 'app/modules/chat'))).resolves.toBe(join(REPO, 'app/modules/chat/worker/src/index.js'));
  });
