...
| ⑧ | 本地签发一次性 setup token | 本地生成 → `wrangler d1 execute` 插入 core 库 `setup_tokens`（幂等：未消费则复用；已封死则跳过），部署输出末尾打印激活链接（#165 方案 B：不再有公开签发端点） |
...
覆盖式；registry upsert 收敛到同一终态；setup token 未消费则复用、实例已封死后
跳过并提示「实例已配置」。**空 CF 账号连跑两次结果一致**（#14 验收）。
...
- **OAuth（推荐）**：`wrangler login` 浏览器授权，零复制粘贴。**2026-09-17 用 wrangler 4.129.0 实测**（原始记录已迁入 [issue #241](https://github.com/UN-Self/unself/issues/241)）：OAuth 覆盖 D1 建库/读写/迁移、R2 建桶/列举、KV 建命名空间、Workers 部署、secret 写入、zone 路由增/改/删；**唯一不可用的是 Total TLS（ACM）**，只有多级子域（比 zone 深两级以上）才需要它。
