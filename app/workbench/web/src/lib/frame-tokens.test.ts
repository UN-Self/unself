...
describe('tokensStyleSource（通道 A 注入的 style 文本，§6.5.5）', () => {
  ...
  async function shellTokensCss(): Promise<Map<string, string>> {
    // jsdom 的 import.meta.url 是 http(s) 方案，readFile 只收 file:// ——vitest cwd = app/workbench
    const css = await readFile(pathToFileURL(join(process.cwd(), 'web/src/tokens.css')), 'utf8')
  ...
  it('默认包注入文本与壳 tokens.css 逐项同值（契约令牌两处实现不漂移）', async () => {
    ...
    // 对照真值 = app/workbench/web/src/tokens.css 的 :root 声明（壳自身启动依赖的独立来源）。
